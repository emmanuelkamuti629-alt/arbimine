...
// -------------------- Routes: Public Exchange Data (unchanged) --------------------
...
// -------------------- Paystack Subscription (unchanged) --------------------
...
// In-memory token store (for demo; use JWT in production)
const adminTokens = new Map();

// Create default admin if none exists (run once)
...
app.get('/api/messages/:userId', async (req, res) => {
  try {
    ...
    // Only return messages that are not deleted (soft delete)
...
app.post('/api/messages', async (req, res) => {
  try {
    ...
    // Validate that from/to are either 'admin' or a valid userId
...
app.put('/api/messages/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { content, userId } = req.body; // userId is the sender
...
io.on('connection', (socket) => {
  ...
  // Join a room based on user ID or 'admin'
...
// -------------------- Serve Admin Page --------------------
// We'll serve a basic admin HTML from /admin
>>>>>>> 4a22882 (Your commit message)
