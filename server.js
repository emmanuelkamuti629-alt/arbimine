...
async function safeGet(url, name) {
  try {
    ...
    console.log(`${name} fallback FAILED:`, e.message);
    return null;
  }
}
    ...
    if (exchange === 'binance' && symbol.endsWith('USDT')) {
      ...
      for (const s of possibleSymbols) {
        if (s && s.includes('USDT')) {
          sym = s.replace(/[^a-zA-Z0-9]/g, '').replace('USDT', '');
          price = +t.last || +t.close || +t.lastPrice || +t.a || 0;
          volume = +t.vol || +t.quoteVolume || +t.volValue || +t.v || 0;
          break;
        }
      }
