...
const transactionSchema = new mongoose.Schema({
  ...
  intasendResponse: mongoose.Schema.Types.Mixed,
  createdAt: { type: Date, default: Date.now }
});
...
const APP_URL = process.env.APP_URL || 'https://arbimine-ke.onrender.com';
...
async function intasendRequest(method, endpoint, data = null) {
  const url = `https://api.intasend.com/v1/${endpoint}`;
  const headers = {
...
app.post('/api/intasend/pay', async (req, res) => {
  const token = req.headers.authorization;
  if (!token) return res.status(401).json({ error: 'No token' });
  try {
    const session = await Session.findOne({ token });
    ...
    const payload = {
      ...
      currency: 'KES',
      reference: reference,
      description: `ArbiMine ${plan} subscription`,
      callback_url: `${APP_URL}/api/intasend/webhook`,
      metadata: {
    ...
    const response = await intasendRequest('POST', 'payment/mpesa-stk-push/', payload);
...
app.post('/api/intasend/webhook', async (req, res) => {
  ...
  try {
    const { reference, status, metadata } = event;
    ...
    if (status === 'success') {
      ...
      if (username && plan) {
        ...
        console.log(`Subscription activated for ${username} (${plan}) via webhook`);
      }
    } else {
...
app.get('/api/intasend/verify/:reference', async (req, res) => {
