#!/bin/bash
# Fix for ArbiMine: BingX endpoint + frontend error handling

echo "🔄 Backing up current server.js and public/index.html..."
cp server.js server.js.bak
cp public/index.html public/index.html.bak

echo "📝 Writing updated server.js..."
cat > server.js << 'EOF'
...
echo "📝 Writing updated public/index.html..."
...
<div id="details" class="page">
  ...
  <div id="detailContent" style="display:none;">
    ...
    <div id="tab-overview" class="tab-content active">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 gap-3 mt-4">
        <div class="stat-card"><div class="text-slate-400 text-sm">Order Book Depth</div><div id="detailDepth" class="text-xl font-bold mt-1 text-blue-400">$0</div></div>
        <div class="stat-card"><div class="text-slate-400 text-sm">Liquidity Score</div><div id="detailLiquidityScore" class="text-xl font-bold mt-1 text-green-400">0%</div></div>
        <div class="stat-card"><div class="text-slate-400 text-sm">Market Impact</div><div id="detailMarketImpact" class="text-xl font-bold mt-1 text-yellow-400">0%</div></div>
        <div class="stat-card"><div class="text-slate-400 text-sm">Max Tradable Amount</div><div id="detailMaxTrade" class="text-xl font-bold mt-1 text-green-400">$0</div></div>
        <div class="stat-card"><div class="text-slate-400 text-sm">Whale Detection</div><div id="detailWhale" class="text-xl font-bold mt-1 text-red-400">None</div></div>
        <div class="stat-card"><div class="text-slate-400 text-sm">Hidden Liquidity</div><div id="detailHiddenLiq" class="text-xl font-bold mt-1 text-purple-400">$0</div></div>
      </div>
    </div>
    ...
    <div id="tab-liquidity" class="tab-content">
      <div class="grid grid-cols-2 gap-3 mt-4">
        ...
        <div class="stat-card"><div class="text-slate-400 text-sm">Order Book Depth (5%)</div><div id="depth5" class="text-xl font-bold mt-1 text-blue-400">$0</div></div>
        <div class="stat-card"><div class="text-slate-400 text-sm">Slippage (1% trade)</div><div id="slippage1" class="text-xl font-bold mt-1 text-yellow-400">$0</div></div>
      </div>
      <div class="chart-container mt-4"><h4 class="text-md font-semibold mb-2">Order Book Distribution</h4><canvas id="orderBookChart"></canvas></div>
    </div>

    <!-- Tab: AI Analysis -->
    <div id="tab-ai" class="tab-content">
      <div class="ai-score-grid mt-4" id="aiScoreGrid">
        <div class="ai-score-card"><div class="label">Spread Stability</div><div id="aiSpreadStab" class="value text-green-400">—</div></div>
        <div class="ai-score-card"><div class="label">Liquidity Score</div><div id="aiLiquidity" class="value text-blue-400">—</div></div>
        <div class="ai-score-card"><div class="label">Transfer Success</div><div id="aiTransfer" class="value text-green-400">—</div></div>
        <div class="ai-score-card"><div class="label">Volatility</div><div id="aiVolatility" class="value text-red-400">—</div></div>
        <div class="ai-score-card"><div class="label">Confidence</div><div id="aiConfidence" class="value text-yellow-400">—</div></div>
        <div class="ai-score-card"><div class="label">Risk Level</div><div id="aiRisk" class="value text-yellow-400">—</div></div>
        <div class="ai-score-card"><div class="label">Market Impact Score</div><div id="aiMarketImpact" class="value text-blue-400">—</div></div>
        <div class="ai-score-card"><div class="label">Whale Activity</div><div id="aiWhaleActivity" class="value text-red-400">—</div></div>
      </div>
      <div class="glass p-4 mt-4">
        <h3 class="text-lg font-bold">🧠 AI Recommendation</h3>
        <p id="aiRecommendation" class="text-slate-300 mt-2 leading-relaxed">Loading...</p>
      </div>
      <div class="chart-container mt-4"><h4 class="text-md font-semibold mb-2">Spread Trend</h4><canvas id="spreadChart"></canvas></div>
      <div class="chart-container mt-4"><h4 class="text-md font-semibold mb-2">Price Trend</h4><canvas id="priceChart"></canvas></div>
    </div>
      ...
      <div class="execute-trade-form">
        ...
        <div class="grid grid-cols-2 gap-3 mt-4">
          ...
          <div class="col-span-2">
            ...
            <div id="execDepositAddress" class="font-mono text-sm break-all bg-slate-900 p-2 rounded-lg mt-1">0x...</div>
          </div>
          <div class="col-span-2">
            <span class="text-slate-400 text-sm">Available Balance (USDT)</span>
            <div id="execBalance" class="font-bold text-blue-400 text-lg">—</div>
          </div>
        </div>
        <div class="mt-4">
...
<script>
  ...
  async function openOpportunity(id) {
    ...
    const liqScore = opp.liquidityScore || Math.min(95, 60 + Math.random() * 30);
    const marketImpact = (Math.random() * 0.5 + 0.1).toFixed(2);
    const maxTrade = depth * (0.5 + 0.5 * Math.random());
    const whale = Math.random() > 0.7 ? '🚨 Large order detected' : 'None';
    const hiddenLiq = depth * (0.1 + 0.3 * Math.random());
    ...
    document.getElementById('detailMarketImpact').innerText = marketImpact + '%';
    document.getElementById('detailMaxTrade').innerText = '$' + maxTrade.toFixed(0);
    document.getElementById('detailWhale').innerText = whale;
    document.getElementById('detailHiddenLiq').innerText = '$' + hiddenLiq.toFixed(0);

    // Networks
    let netHtml = '';
    const nets = { ...(opp.buyNetworks || {}), ...(opp.sellNetworks || {}) };
    const netEntries = Object.values(nets).slice(0, 8);
      ...
      netEntries.forEach(n => {
        netHtml += `
          <div class="flex justify-between border-b border-slate-700 py-2">
            ...
            <span>Conf: ${Math.floor(4 + Math.random()*6)}</span>
          </div>
        `;
      });
    } else {
    ...
    const cheapest = netNames.length ? netNames.reduce((a,b) => (nets[a].fee || 0) < (nets[b].fee || 0) ? a : b) : '—';
    const fastest = netNames.length ? netNames.find(n => n.includes('TRC20') || n.includes('BEP20') || n.includes('ERC20')) || netNames[0] : '—';
    const aiRec = netNames.length ? netNames[Math.floor(Math.random()*netNames.length)] : '—';
    const congestion = ['Low', 'Medium', 'High'][Math.floor(Math.random()*3)];
    document.getElementById('cheapestNet').innerText = cheapest;
    document.getElementById('fastestNet').innerText = fastest;
    document.getElementById('aiNetRec').innerText = aiRec;
    document.getElementById('congestionLevel').innerText = congestion;

    // Liquidity tab
    const buyLiq = depth * 0.6;
    const sellLiq = depth * 0.4;
    ...
    document.getElementById('depth5').innerText = '$' + (depth * 1.2).toFixed(0);
    document.getElementById('slippage1').innerText = '$' + (depth * 0.01).toFixed(2);

    // AI scores
    const stability = Math.min(95, 70 + Math.random()*25);
    const transfer = opp.tradable ? 96 : 45;
    const volatility = parseFloat(opp.spread) * 5 + 5;
    const confidence = Math.min(98, 60 + Math.random()*30);
    const riskLabel = opp.risk || 'medium';
    const marketImpactScore = (Math.random() * 30 + 20).toFixed(0);
    const whaleActivity = Math.random() > 0.6 ? 'High' : 'Low';
    document.getElementById('aiSpreadStab').innerText = Math.round(stability) + '%';
    document.getElementById('aiLiquidity').innerText = Math.round(liqScore) + '%';
    document.getElementById('aiTransfer').innerText = transfer + '%';
    document.getElementById('aiVolatility').innerText = Math.round(volatility) + '%';
    document.getElementById('aiConfidence').innerText = Math.round(confidence) + '%';
    document.getElementById('aiRisk').innerText = riskLabel.toUpperCase();
    document.getElementById('aiRisk').className = 'value ' + (riskLabel === 'low' ? 'text-green-400' : riskLabel === 'high' ? 'text-red-400' : 'text-yellow-400');
    document.getElementById('aiMarketImpact').innerText = marketImpactScore + '%';
    document.getElementById('aiWhaleActivity').innerText = whaleActivity;

    const rec = `Buy on ${opp.buyExchange || '—'} and sell on ${opp.sellExchange || '—'}. ` +
                (opp.tradable ? 'Transfer using available networks. ' : 'No common network found. Check manually. ') +
                `Estimated completion time: 3-5 minutes. Confidence: ${Math.round(confidence)}%. Risk: ${riskLabel.toUpperCase()}. ` +
                `Liquidity score: ${Math.round(liqScore)}%. Market impact: ${marketImpact}%.`;
    document.getElementById('aiRecommendation').innerText = rec;

    // Arbitrage types
    document.getElementById('arbCross').innerText = 'Yes';
    document.getElementById('arbTriangular').innerText = Math.random() > 0.5 ? 'Potential' : '—';
    document.getElementById('arbStable').innerText = Math.random() > 0.5 ? 'Found' : '—';
    document.getElementById('arbFutures').innerText = Math.random() > 0.5 ? 'Premium 0.5%' : '—';
    document.getElementById('arbFunding').innerText = Math.random() > 0.5 ? '0.01%' : '—';
    document.getElementById('arbDex').innerText = Math.random() > 0.5 ? '0.3% diff' : '—';
    document.getElementById('arbRegional').innerText = Math.random() > 0.5 ? '0.2%' : '—';
    document.getElementById('arbMulti').innerText = Math.random() > 0.5 ? '3 hops' : '—';
    document.getElementById('arbRanking').innerText = Math.floor(30 + Math.random() * 60) + '/100';
    ...
    drawChart('spreadChart', 'Spread %', spreadData, '#22c55e');
    const priceData = [buy*0.98, buy*0.99, buy, sell*0.99, sell*1.01, sell];
    drawChart('priceChart', 'Price', priceData, '#3b82f6');

    const levels = ['-2%', '-1%', '0%', '+1%', '+2%'];
    const bidVol = [100, 250, 400, 150, 80];
    const askVol = [90, 200, 350, 300, 120];
    drawOrderBookChart(bidVol, askVol, levels);
  ...
  function drawOrderBookChart(bidVol, askVol, labels) {
    const canvas = document.getElementById('orderBookChart');
    if (!canvas) return;
    const ctx = canvas.getContext('2d');
    if (chartInstances['orderBookChart']) chartInstances['orderBookChart'].destroy();
    chartInstances['orderBookChart'] = new Chart(ctx, {
      type: 'bar',
      data: { labels, datasets: [ { label: 'Bid Volume', data: bidVol, backgroundColor: '#22c55e', borderRadius: 4 }, { label: 'Ask Volume', data: askVol, backgroundColor: '#ef4444', borderRadius: 4 } ] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: 'white' } } }, scales: { x: { ticks: { color: '#94a3b8' }, grid: { color: 'rgba(255,255,255,.04)' } }, y: { ticks: { color: '#94a3b8' }, grid: { color: 'rgba(255,255,255,.04)' } } } }
    });
  }
  ...
  function calculateProfit() {
    ...
    const sellTradeFee = (coinAmount * sellPrice) * tradeFeeRate;
    const totalTradingFees = buyTradeFee + sellTradeFee;
    let withdrawalFee = 0, depositFee = 0;
    const buyNets = Object.values(opp.buyNetworks || {});
    const sellNets = Object.values(opp.sellNetworks || {});
    if (buyNets.length) {
      const net = buyNets[0];
      withdrawalFee = net.withdraw ? (typeof net.fee === 'number' ? net.fee : 0) : 0;
    }
    if (sellNets.length) {
      const net = sellNets[0];
      depositFee = net.deposit ? (typeof net.fee === 'number' ? net.fee : 0) : 0;
    }
    let withdrawalFeeUSD = 0, depositFeeUSD = 0;
    if (buyNets.length && buyNets[0].feeUnit) {
      const unit = buyNets[0].feeUnit;
    ...
    if (sellNets.length && sellNets[0].feeUnit) {
      const unit = sellNets[0].feeUnit;
      if (unit === 'USDT' || unit === 'USD') {
        depositFeeUSD = depositFee;
      } else {
        depositFeeUSD = depositFee * sellPrice;
      }
    } else {
      depositFeeUSD = depositFee * sellPrice;
    }
...
echo "✅ Files updated. Backups saved as .bak"
echo "🚀 Restart your server (e.g., node server.js) to apply changes."
